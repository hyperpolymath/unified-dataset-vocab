...
│  generated/abi/unified_dataset_vocab.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libunified_dataset_vocab.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── unified_dataset_vocab.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── unified_dataset_vocab.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/unified_dataset_vocab.h
```
...
```c
#include "unified_dataset_vocab.h"

int main() {
    void* handle = unified_dataset_vocab_init();
    if (!handle) return 1;

    int result = unified_dataset_vocab_process(handle, 42);
    if (result != 0) {
        const char* err = unified_dataset_vocab_last_error();
...
Compile with:
```bash
gcc -o example example.c -lunified_dataset_vocab -L./zig-out/lib
```

### From Idris2

```idris
import UnifiedDatasetVocab.ABI.Foreign
...
```rust
#[link(name = "unified_dataset_vocab")]
extern "C" {
    fn unified_dataset_vocab_init() -> *mut std::ffi::c_void;
    fn unified_dataset_vocab_free(handle: *mut std::ffi::c_void);
    fn unified_dataset_vocab_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = unified_dataset_vocab_init();
        assert!(!handle.is_null());

        let result = unified_dataset_vocab_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libunified_dataset_vocab = "libunified_dataset_vocab"

function init()
    handle = ccall((:unified_dataset_vocab_init, libunified_dataset_vocab), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:unified_dataset_vocab_process, libunified_dataset_vocab), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:unified_dataset_vocab_free, libunified_dataset_vocab), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/unified_dataset_vocab.h
   ```
